...
- 可折叠、可调整宽度的桌面侧栏
- 概览、下载任务、网络与设置四个顶层入口；窄屏自动切换为底部导航
- 下载任务页内提供全部、正在传输、已完成、需要处理四种紧凑筛选
- 概览只汇总现有任务与引擎真实数据，不伪造上传、历史趋势或插件能力
- 网络页展示本机引擎、默认下载目录、Peer 预算与受限 BT 能力状态
- 宽屏任务列表 + 点击任务后按需展开、可关闭的详情检查器，窄屏独立详情页
- 快速添加任务、全局搜索、键盘快捷键
- 浅色、深色、跟随系统和中英文
...
- 速度、已下载、总大小、剩余时间、连接数
- 排序、筛选、搜索、多选批量操作
- 已完成任务可手动打开或定位，并可选择完成后自动在文件管理器中显示
- 桌面端任务行右键菜单按状态提供详情、暂停/继续、失败重试、取消、文件操作和删除记录
...
- 空间预检查、临时文件、原子完成
- HTTP 重名时停止或自动生成副本名（已完成）；移动、校验和自动解压待实现
- 完成通知、Webhook；任意脚本执行不进入 MVP

### 8. 设置与诊断

- 下载目录、速度与调度、BT、外观、通知和工作区设置
- 引擎状态、版本、脱敏数据位置、日志保留状态和诊断导出（已完成首个闭环）
- 数据备份、恢复和任务数据库迁移
- 第三方许可证与隐私说明
...
- macOS 保留原生窗口按钮并让内容延伸到透明标题栏；深色界面使用靛紫到海军蓝的整窗背景、同色侧栏和带靛蓝色相的午夜色内容表面，主操作使用墨色/反色，状态使用独立语义色。
...
- 任务列表是主体，不把每个任务包装成厚重悬浮卡片；列表使用独立的雾灰/午夜蓝工作表面，桌面搜索框固定为 `280 × 40`，任务行以文件名为第一层，状态、分段轨道、传输量、速度和百分比使用稳定列网格，窄宽度下按信息层级自然换行。
...
```text
应用外壳
├── 概览：实时总下载速度 / 任务状态汇总 / 当前传输 / 最近任务
├── 下载任务
│   ├── 页内筛选：全部 / 正在传输 / 已完成 / 需要处理
│   ├── 工具栏：添加 / 搜索 / 排序 / 批量操作
│   ├── 任务列表：名称 / 轨道 / 速度 / 大小 / 剩余时间
│   ├── 详情检查器：宽屏按需展开，窄屏使用独立详情页
│   └── 新建下载模态框：链接检查 / 保存位置 / 创建结果
├── 网络：引擎信息 / 下载边界 / BT 受限网络策略
└── 设置：本机界面与系统偏好
...
`POST /api/v1/diagnostics/export` 成功时直接返回 `application/zip`；失败仍使用统一 JSON 错误包络。
...
新建 HTTP 下载会自动填入引擎设置中的默认下载目录，因此用户解析链接后可直接开始下载；“更改位置”仍只覆盖当前新建流程，不会修改全局默认值。设置页的“下载与文件”分组可通过系统目录选择器更新全局默认目录，并在“自动重命名”与“停止创建”之间选择重名策略；更新失败时保留服务端原值并显示本地化错误。自动重命名同时避开已有最终文件、`.downpeed` 临时文件和排队/下载/重试/暂停任务占用的路径，只影响之后创建的 HTTP 任务，不移动已有任务或本地文件，也不改变 BT 的严格冲突行为。该分组还提供本机“下载完成后”动作，默认无操作，可选择在系统文件管理器中显示完成文件。

设置页的“速度与调度”分组可调整最大并发任务、全局下载限速和自动重试次数；控件保存期间禁用，失败时保留引擎原值并显示独立错误。网络页同步显示本机引擎当前执行的三项调度值。“数据与诊断”分组显示缩写后的数据/数据库位置、数据库大小、日志保留状态和任务计数，并支持选择位置导出 ZIP 以及在文件管理器中定位保存结果。桌面使用设置导航与单一内容区，窄屏继续采用菜单到详情的主从导航，并覆盖 200% 文字缩放。

任务工作台可按全部、正在传输、已完成和需要处理筛选，并按最新、最早、文件名、进度或体积排序；搜索覆盖文件名、URL 和保存目录。多选命令条只提交状态允许的任务，成功项立即以引擎响应回写，失败项保留选择以便用户继续处理。

可安全重试的失败任务会在列表快捷操作、右键菜单和详情面板显示“重试”。Flutter 调用 `POST /api/v1/tasks/{id}/retry` 后只采用 Go 内核回读的任务状态；内核会再次检查最终文件、临时文件和分段检查点，保留经过校验的可续传数据，否则从 0 重新排队。资源已变化或错误被标记为不可重试时不会显示重试入口；若操作时才发现目标文件冲突，内核也会拒绝重试。不可重试错误使用稳定的 `task_not_retryable`，避免拼接错误内容或覆盖现有文件。
...
Flutter 只在已知任务通过 SSE 从非完成态切换为 `completed` 时发送一次系统通知并处理可选完成动作；启动、刷新或断线重连加载到的历史完成任务不会重复触发。完成动作作为当前设备偏好保存且默认关闭；开启后会在文件管理器中显示完成文件，450ms 内多项任务完成时只定位最后一项，避免连续打开多个窗口，通知仍逐项发送。已完成任务也可从列表快速打开文件，或在详情中手动打开和定位。桌面动作通过 MethodChannel 调用 macOS `NSWorkspace`、Windows Shell API，以及 Linux GIO/FreeDesktop API；原生层再次检查绝对路径和文件存在性，不使用 Shell 拼接路径。通知被拒绝不会改变任务状态，文件被移动、删除或系统无法打开时，Flutter 显示本地化且不包含敏感路径的反馈。

新任务先进入 Go 内核的 FIFO 队列，获得并发槽位后从 `queued` 切换为 `downloading`。默认最多同时运行 3 个任务；运行时提高并发上限会立即补充启动排队任务，降低上限不会停止已经占用槽位的传输，只会阻止新任务启动直至运行数回落。排队、下载中和等待重试的任务都可暂停或取消。HTTP `408`、`429`、`5xx`、网络超时和响应截断等临时故障会释放并发槽位，进入 `retrying`，默认最多重试 2 次，等待时间为 1 秒、2 秒的指数退避；运行时修改重试次数会作用于之后的故障判定。权限拒绝、Range 不一致、本地文件变化和发布失败不会盲目重试。任务响应通过 `retryCount` 与 `nextRetryAt` 暴露可解释的重试状态。

全局下载限速由所有单连接和分段连接共享同一个可动态更新的令牌桶，`0` 表示不限速；修改会唤醒正在等待带宽的连接并立即采用新值。`--max-concurrent-tasks`、`--max-retries` 和以 bytes/s 为单位的 `--download-rate-limit` 提供首次运行或旧数据迁移时的默认值，之后以 bbolt 中的引擎设置为准；`--retry-base-delay` 仍是启动级参数。设置更新先持久化再应用到运行时，保存失败不会改变当前调度器。限速等待响应暂停、取消和服务关闭，不会阻塞任务状态切换。

下载数据写入最终文件同目录的隐藏临时文件 `.文件名.downpeed`，下载中和暂停时最终路径保持不存在。创建 HTTP 任务时的自动重命名只负责选择当时未占用的候选路径；完整内容 `Sync` 成功后仍通过同目录硬链接原子发布最终路径并清理临时目录项。如果选择名称后外部程序又创建了同名最终文件，发布会安全失败且不会覆盖该文件。取消和普通初始下载失败会清理临时文件，暂停会保留分片供 Range 继续。
