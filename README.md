...
- 原创品牌标记以三条“传输轨道”汇聚为向下落点，平台图标规范与母版见 [品牌说明](docs/brand/README.md)。
- macOS 保留原生窗口按钮并让内容延伸到透明标题栏；主操作使用墨色/反色，状态使用独立语义色。
- 使用平台系统字体与克制的 400/500/600 字重层级，中文优先 PingFang SC，数据使用等宽数字。
...
桌面侧栏支持展开、图标化收起和鼠标拖拽调节宽度，展开状态与宽度作为本机界面偏好保存；窗口空间不足时自动切换为紧凑底部导航。侧栏设置入口打开独立设置页，可切换主题、语言和默认侧栏状态，不在 Flutter 中复制下载业务配置。
