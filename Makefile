...
license-check: ## 校验已批准的 BT 候选组件证据
	@cd $(BACKEND_DIR) && $(GO) run ./cmd/licensecheck -mode candidate

check: ## 执行格式、静态分析、单测、竞态和 Go 构建检查
...
engine: go-deps ## 启动 Go 下载引擎（阻塞当前终端）
	@cd $(BACKEND_DIR) && $(GO) run ./cmd/downpeedd $(ENGINE_FLAGS)
...
build-engine-macos: ## 编译 macOS Go 引擎
	...
	@cd $(BACKEND_DIR) && CGO_ENABLED=0 GOOS=darwin GOARCH=$(TARGET_ARCH) $(GO) build \
...
build-engine-windows: ## 编译 Windows Go 引擎
	...
	@cd $(BACKEND_DIR) && CGO_ENABLED=0 GOOS=windows GOARCH=$(TARGET_ARCH) $(GO) build \
		-trimpath -ldflags='$(GO_LDFLAGS)' \
...
build-engine-linux: ## 编译 Linux Go 引擎
	...
	@cd $(BACKEND_DIR) && CGO_ENABLED=0 GOOS=linux GOARCH=$(TARGET_ARCH) $(GO) build \
		-trimpath -ldflags='$(GO_LDFLAGS)' \
		-o $(BUILD_DIR)/engine/linux-$(TARGET_ARCH)/downpeedd ./cmd/downpeedd

build: build-$(HOST_OS) ## 构建当前平台 Release 客户端与引擎

build-macos: flutter-deps build-engine-macos ## 构建 macOS Release
...
build-windows: flutter-deps build-engine-windows ## 构建 Windows Release
	@if [ '$(HOST_OS)' != windows ]; then echo 'Windows Flutter 包必须在 Windows 上构建' >&2; exit 1; fi
	@cd $(APP_DIR) && $(FLUTTER) build windows --release \
		--build-name=$(VERSION) --build-number=$(BUILD_NUMBER) \
		--dart-define=DOWNPEED_API_BASE_URL=$(API_BASE_URL)

build-linux: flutter-deps build-engine-linux ## 构建 Linux Release
...
package-macos: build-macos ## 生成 macOS DMG（未签名、未公证）
	...
	@install -m 0755 '$(BUILD_DIR)/engine/macos-$(TARGET_ARCH)/downpeedd' \
		'$(DIST_DIR)/downpeed-$(VERSION)-macos-$(TARGET_ARCH)/downpeedd'
	@install -m 0755 '$(ROOT_DIR)/scripts/start-downpeed-unix.sh' \
		'$(DIST_DIR)/downpeed-$(VERSION)-macos-$(TARGET_ARCH)/Start Downpeed.command'
...
package-windows: build-windows ## 生成 Windows 便携 ZIP
	...
	@cp '$(BUILD_DIR)/engine/windows-$(TARGET_ARCH)/downpeedd.exe' \
		'$(DIST_DIR)/downpeed-$(VERSION)-windows-$(TARGET_ARCH)/downpeedd.exe'
	@cp '$(ROOT_DIR)/scripts/start-downpeed-windows.ps1' '$(ROOT_DIR)/scripts/start-downpeed-windows.cmd' \
		'$(DIST_DIR)/downpeed-$(VERSION)-windows-$(TARGET_ARCH)/'
...
package-linux: build-linux ## 生成 Linux 便携 tar.gz
	...
	@install -m 0755 '$(BUILD_DIR)/engine/linux-$(TARGET_ARCH)/downpeedd' \
		'$(DIST_DIR)/downpeed-$(VERSION)-linux-$(TARGET_ARCH)/downpeedd'
	@install -m 0755 '$(ROOT_DIR)/scripts/start-downpeed-unix.sh' \
		'$(DIST_DIR)/downpeed-$(VERSION)-linux-$(TARGET_ARCH)/start-downpeed.sh'
